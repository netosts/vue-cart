    ...
    <div class="row tw-flex tw-w-full tw-justify-center tw-items-center tw-py-4">
      ...
      <q-card-section class="tw-flex tw-justify-between">
        <q-card-section class="tw-flex tw-gap-6">
          <q-img
        fit="contain"
          class="tw-w-[200px] tw-h-[150px] tw-flex tw-mt-2"
          :src="cartValue[0].image"
        />
        <div class="tw-flex tw-flex-col tw-justify-between">
          ...
          <div class="tw-flex tw-items-center tw-gap-2">
            <q-icon
              ...
              @click="!isMinimumAmount(cartValue) ? removeFromCart(cartValue[0]) : null"
              size="sm"
              :class="isMinimumAmount(cartValue) ? 'text-grey-5' : 'tw-cursor-pointer'"
            />
            <div :class="isMaximumAmount(cartValue) ? 'text-grey-5 tw-cursor-default' : 'text-default'">{{ productAmount(cartValue) }}</div>
            <q-icon
              name="add_circle_outline"
              @click="!isMaximumAmount(cartValue) ? addToCart(cartValue[0]) : null"
              size="sm"
              :class="isMaximumAmount(cartValue) ? 'text-grey-5' : 'tw-cursor-pointer'"
            />
            <q-separator vertical></q-separator>
            <q-icon name="delete" @click="deleteFromCart(cartValue[0])" size="sm" class="tw-cursor-pointer"></q-icon>
          </div>
        </div>
